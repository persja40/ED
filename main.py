...
def gower_affinity(X):
        return pairwise_distances(X, metric=gower_distance)

def make_cluster(params):
    (cluster_nr, filtered_samples, encoder_dict) = params
    # print(cluster_nr)
    # print(filtered_samples)
    # print(encoder_dict)
    cluster = AgglomerativeClustering(
        n_clusters=cluster_nr, affinity=gower_affinity, linkage="average")

    data_clusters = [[] for i in range(cluster_nr)]
    cluster_label = cluster.fit_predict(filtered_samples)
    for i in range(len(filtered_samples)):
        data_clusters[cluster_label[i]].append(filtered_samples[i])

    # # print(data_clusters)
    # for e in data_clusters:
    #     print(len(e))
    for i in range(cluster_nr):
        with open("stats_cl_"+ str(cluster_nr) +"/cluster_"+str(i)+".dat", "w") as f:
            for e in data_clusters[i]:
                f.write(encoder_dict['gender'].inverse_transform([e[0]])[0] + " , " + encoder_dict['race'].inverse_transform([e[1]])[0] + " , " + encoder_dict['parents_edu'].inverse_transform([e[2]])[0] + " , " + encoder_dict['lunch'].inverse_transform([e[3]])[0] +
                        " , " + encoder_dict['prep_test'].inverse_transform([e[4]])[0] + " , " + str(e[5]) + " , " + str(e[6]) + " , " + str(e[7])+"\n")
...
if __name__ == "__main__":
    ...
    # CLUSTER
    params = []
    for cl_n in range(3,11):
        params.append((cl_n, filtered_samples, encoder_dict))
    with mp.Pool(8) as p:
        p.map(make_cluster, params)
    

"""
# CLUSTER
for e in filtered_samples:
    e[1] = 0
print("SUCCESS")
